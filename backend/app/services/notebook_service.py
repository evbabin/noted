...
async def create_notebook(db: AsyncSession, workspace_id: uuid.UUID, data: NotebookCreate) -> Notebook:
...
async def update_notebook(db: AsyncSession, notebook_id: uuid.UUID, data: NotebookUpdate) -> Notebook:
...
async def reorder_notebooks(db: AsyncSession, workspace_id: uuid.UUID, ordered_ids: list[uuid.UUID]) -> None:
    result = await db.execute(
        select(Notebook.id)
        .where(Notebook.workspace_id == workspace_id)
    )
