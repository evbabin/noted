...
async def create_auth_ws_nb(client: AsyncClient, email="test@example.com"):
    ...
    nb = await client.post(
        f"/api/v1/workspaces/{ws_id}/notebooks", json={"title": "NB"}, headers=headers
    )
...
async def test_search_returns_ranked_results(client: AsyncClient):
    ...
    await client.post(
        f"/api/v1/notebooks/{nb_id}/notes",
        json={
            "title": "React Guide",
            "content": {
                "type": "doc",
                "content": [
                    {
                        "type": "paragraph",
                        "content": [{"type": "text", "text": "React is a UI library."}],
                    }
                ],
            },
        },
                ...
                "content": [
                    {
                        "type": "paragraph",
                        "content": [
                            {"type": "text", "text": "Vue is another UI library."}
                        ],
                    }
                ],
            },
    ...
    resp = await client.get(
        f"/api/v1/workspaces/{ws_id}/search", params={"q": "React"}, headers=headers
    )
...
async def test_search_scoped_to_workspace(client: AsyncClient):
    ...
    await client.post(
        f"/api/v1/notebooks/{nb_id1}/notes",
        json={"title": "Secret Python"},
        headers=headers1,
    )

    resp = await client.get(
        f"/api/v1/workspaces/{ws_id2}/search", params={"q": "Python"}, headers=headers2
    )
    assert resp.status_code == 200
...
async def test_search_highlights_matches(client: AsyncClient):
    ...
    await client.post(
        f"/api/v1/notebooks/{nb_id}/notes",
        json={"title": "Animals", "content": content},
        headers=headers,
    )

    resp = await client.get(
        f"/api/v1/workspaces/{ws_id}/search", params={"q": "fox"}, headers=headers
    )
    assert resp.status_code == 200
    data = resp.json()
    assert data["total"] == 1
    assert (
        "<b>fox</b>" in data["results"][0]["snippet"]
        or "fox" in data["results"][0]["snippet"]
    )  # Postgres ts_headline usually uses <b>
