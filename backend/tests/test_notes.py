...
async def test_create_note_with_json_content(client: AsyncClient):
    ...
    resp = await client.post(
        ...
        json={"title": "Note 1", "content": content},
        headers=headers,
    )
...
async def test_update_creates_version(client: AsyncClient):
    ...
    note = await client.post(
        f"/api/v1/notebooks/{nb_id}/notes", json={"title": "Note 1"}, headers=headers
    )
    note_id = note.json()["id"]

    resp = await client.patch(
        f"/api/v1/notes/{note_id}", json={"title": "Updated Note 1"}, headers=headers
    )
...
async def test_plain_text_mirror_updated(client: AsyncClient):
    ...
    content = {
        "type": "doc",
        "content": [
            {"type": "paragraph", "content": [{"type": "text", "text": "Hello Search"}]}
        ],
    }
    note = await client.post(
        f"/api/v1/notebooks/{nb_id}/notes",
...
async def test_version_history_ordered(client: AsyncClient):
    headers, ws_id, nb_id = await create_auth_ws_nb(client)
    note = await client.post(
        f"/api/v1/notebooks/{nb_id}/notes", json={"title": "v1"}, headers=headers
    )
    note_id = note.json()["id"]
    await client.patch(
        f"/api/v1/notes/{note_id}", json={"title": "v2"}, headers=headers
    )
    await client.patch(
        f"/api/v1/notes/{note_id}", json={"title": "v3"}, headers=headers
    )
