...
async def create_auth_and_workspace(client: AsyncClient, email="test@example.com"):
    ...
    ws_resp = await client.post("/api/v1/workspaces/", json={"name": "WS"}, headers=headers)
...
async def test_create_notebook(client: AsyncClient):
    ...
    resp = await client.post(f"/api/v1/workspaces/{ws_id}/notebooks", json={"title": "Notebook 1"}, headers=headers)
    assert resp.status_code == 201
    assert resp.json()["title"] == "Notebook 1"

async def test_reorder_notebooks(client: AsyncClient):
    headers, ws_id = await create_auth_and_workspace(client)
    nb1 = await client.post(f"/api/v1/workspaces/{ws_id}/notebooks", json={"title": "NB1"}, headers=headers)
    nb2 = await client.post(f"/api/v1/workspaces/{ws_id}/notebooks", json={"title": "NB2"}, headers=headers)
    nb1_id = nb1.json()["id"]
    nb2_id = nb2.json()["id"]

    resp = await client.post(f"/api/v1/workspaces/{ws_id}/notebooks/reorder", json={"ordered_ids": [nb2_id, nb1_id]}, headers=headers)
    assert resp.status_code == 200

    list_resp = await client.get(f"/api/v1/workspaces/{ws_id}/notebooks", headers=headers)
    nbs = list_resp.json()
...
async def test_cross_workspace_access_denied(client: AsyncClient):
    ...
    list_resp = await client.get(f"/api/v1/workspaces/{ws_id1}/notebooks", headers=headers2)
