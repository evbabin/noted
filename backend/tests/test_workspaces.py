...
async def test_list_only_member_workspaces(client: AsyncClient):
    ...
    ws_resp1 = await client.post(
        "/api/v1/workspaces/", json={"name": "WS1"}, headers=headers1
    )
    ...
    ws_resp2 = await client.post(
        "/api/v1/workspaces/", json={"name": "WS2"}, headers=headers2
    )
...
async def test_delete_workspace_cascades(client: AsyncClient, register_payload: dict):
    ...
    resp = await client.post(
        "/api/v1/workspaces/", json={"name": "My Workspace"}, headers=headers
    )
    assert resp.status_code == 201
    ws_id = resp.json()["id"]
